...
export function AgentsPage({ initialAgentId }: AgentsPageProps = {}) {
  ...
  // Template de partida para um agente novo. O usuário customiza tudo dentro
  // do próprio Estúdio — isto é só um ponto de partida coerente com o
  // North Radar, em vez de campos vazios.
  const NEW_AGENT_TEMPLATE = {
    description: 'Descreva o propósito deste agente no pipeline North Radar.',
    soul_prompt:
      'Você é um especialista [defina o papel: ex. revisor técnico, redator, analista de risco].\n' +
      'Defina aqui: que elo do pipeline você é (ex. "recebo o output do Agente X e entrego para o Agente Y"),\n' +
      'qual seu tom (executivo, comercial, técnico) e qual princípio guia suas decisões quando há ambiguidade.',
    system_prompt:
      'Sua missão é [objetivo concreto e mensurável deste agente].\n\n' +
      'ESTRUTURA OBRIGATÓRIA DO OUTPUT:\n' +
      '1. [seção 1]\n' +
      '2. [seção 2]\n' +
      '3. [seção 3]\n\n' +
      'Defina também: o que este agente NUNCA deve fazer (delegue ao próximo agente do pipeline).',
    operational_prompt:
      'Se o input do agente anterior não estiver disponível, sinalize isso explicitamente em vez de inventar contexto.\n' +
      'Contexto do operador (North Radar): foco em sites institucionais, landing pages, sistemas web/SaaS,\n' +
      'dashboards, APIs, automações/integrações e plataformas multi-tenant — ajuste esta linha ao papel do agente.\n' +
      'Output em markdown, pronto para o próximo elo do pipeline ou para o cliente final.',
  ...
  const NEW_AGENT_DEFAULT_TOOLS = ['filesystem', 'terminal', 'markdown_export'];
  ...
  const createAgent = async () => {
    ...
    try {
      ...
      // Habilita as ferramentas padrão do template (upsert individual — não há endpoint em lote).
      await Promise.all(
        NEW_AGENT_DEFAULT_TOOLS.map((tool) => api.agents.setTool(created.id, tool, true)),
      );
  ...
  const exportTeam = async () => {
    ...
    a.href = url;
    a.download = 'north-radar-time-de-agentes.json';
    a.click();
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-[13px] text-secondary mt-1">
                Monte e personalize os agentes de IA do seu pipeline North Radar — Soul, prompts,
                ferramentas e modelo, tudo configurado aqui dentro.
              </p>
            </div>
            <button
              onClick={createAgent}
      ...
      <section className="overflow-y-auto">
        {selected ? (
          ...
          <div className="h-full grid place-items-center text-secondary">
            {agents.length === 0
              ? 'Clique em "Novo" para criar seu primeiro agente — você personaliza tudo aqui dentro do North Radar.'
              : 'Selecione um agente'}
          </div>
        )}
      </section>
